...
Cica などを RStudio で使えるようにするために、Python ツールを使って Cica を当幅フォントとする。以下は、bash で行う。

```{bash eval=FALSE}
% pip3 install fonttools
% ttx Cica-Regular.ttf
```

なお、一行目の後で、以下のような警告が出ることがある。

```
  WARNING: The scripts fonttools, pyftmerge, pyftsubset and ttx are installed in '/Users/baba/Library/Python/3.9/bin' which is not on PATH.
  Consider adding this directory to PATH or, if you prefer to suppress this warning, use --no-warn-script-location.
```

これは、ttx にパスが通っていないということである。その場合は、このように入力する。

```{bash eval=FALSE}
% /Users/ユーザー名/Library/Python/3.9/bin/ttx Cica-Regular.ttf
```

Cica-Regular.ttx ファイルというファイルができている。これをテキストエディタで開き、`<isFixedPitch value="0"/>` を `<isFixedPitch value="1"/>` にする。

つぎに、フォントを作成する。

```{bash eval=FALSE}
% ttx -o Cica-Fixed.ttf -m Cica-Regular.ttf Cica-Regular.ttx
```

こうしてできた Cica-Fixed.ttf をインストールする。Font Book においても、等幅フォントになっているはずである。

RStudio が立ち上がっている場合、いちど終了して立ちあげなおす。RStudio が Cica-Fixed を認識するはずである。フォント名は Cica Regular。
