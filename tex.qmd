...
なお、TinyTex は、Tiny でありながら、インストール直後で 300MB 程度 (2026年確認)、よくつかうパッケージをインストールすると 3GB くらいにはなります。

TinyTex の本体は、Windows では `C:/Users/username/AppData/Roaming/TinyTex` にあります。

このため、外付けディスクなどにインストールすることも可能です。

```{r eval=FALSE}
tinytex::install_tinytex(dir = "E:/TinyTex")
```

通常と異なる場所に保存すると、R で TeX が見つからないことがあります。いろいろ解決方法はありますが、Windows であれば `C:/Users/username/AppData/Roaming/TinyTex` にリンクを張るとよいでしょう。
...
tinytex::tlmgr_install("scheme-full") で、多くの TeX パッケージ (2026年に確認したところ 4899 個だった) をインストールしておくとよいでしょう。これで 3GB くらいになります。
...
TIP `r sprintf("%03d", i)`: ほかに必要なもの
:::

[Ghostscript](https://ghostscript.com/) は必須です。
